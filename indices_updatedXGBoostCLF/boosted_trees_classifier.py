...
class XGBoost_sklearn_CLF():
    ...
    def get_provisional_nonsignals(self,X_unknown_unscaled,
        ...
        print(f"{nonsigcounter/len(data_unknown)*100}% of unknown data classified not a signal")
        
        filename="provisional_signals/XGBClf_nonsignals_"+f"{N_classes}_classes_"+ \
                f"{size_class}_class_size"+".pkl"
                
        store_obtained_nonsigs = nonsigs_provis.to_pickle(filename)
        return nonsigs_provis
    
    def get_provisional_indices(self,X_unknown_unscaled,
        ...
        if signal:
            ...
            print(f"{len(indices)/len(data_unknown)*100}% of unknown data classified as signal")
            ...
            print(f"{len(indices)/len(data_unknown)*100}% of unknown data classified not a signal")
...
signalindices = model.get_provisional_indices(X_unknown_unscaled,
