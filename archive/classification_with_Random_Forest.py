...
# Get keys - names of columns in the pd.DataFrame
...
# Labels - names for each of 11 files to be used later in figures and as labels
dataset_names = ["Total dataset",
                 "Signal",
                 r"J/$\psi$",r"$\psi$(2S)",
...
# Generate labels to be used later for classification
dataset_labels = [i for i in range(len(datasets))]

# Modify pd.DataFrames
#features - "B0_ID" and "year" seemed useless for classification
# Can be adjusted as one wishes

for i in range(len(datasets)):
    # features - "B0_ID" and "year" seemed useless for classification -> removed
    datasets[i] = datasets[i].drop(columns=["year","B0_ID"])
    # Add label to each file (i.e data from sig.pkl get "file"==1)
    datasets[i]["file"] = dataset_labels[i]

# Update keys - list of columns, since I've now modified Dataframes
...
#%% Training on 10 classes (files) that are known

# Merge/append two datasets together
merged = datasets[1].iloc[0:2]

for i in range(1,11):
    # Merge/append data from each file (type of decay channel) to "merged"
    data = datasets[i]
    # I want equal number of files for each class, i.e 1000
    if len(data)>1e3:
        file = data.iloc[0:1000]
...
# (here training data makes up 75% = 9775*0.75 rows of previous X and y
# and testing data the remaining 25% = 9775*0.25 rows)
...
confusion_matrix = RF.get_confusion_matrix(X_test,y_test)
